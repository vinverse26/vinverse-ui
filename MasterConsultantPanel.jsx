...
export default function MasterConsultantPanel({ projectId, onProposedChanges }) {
  ...
  const send = async () => {
    ...
    try {
      ...
      const res = await api.post(url, { message: userMessage, conversation_id: conversationId })
      setConversationId(res.data.conversation_id)
      setMessages((m) => [...m, { role: 'assistant', content: res.data.reply }])
      if (res.data.proposed_state_changes) {
        setLastProposal(res.data.proposed_state_changes)
        onProposedChanges?.(res.data.proposed_state_changes)
      }
    } catch (err) {
  ...
  const acceptProposal = async () => {
    ...
    await api.post(`/projects/${projectId}/state/apply`, lastProposal)
